...
const PWAInstallModal: React.FC = () => {
    ...
    const getInstallSteps = () => {
        if (isDesktop) {
            return [
                ...
                localize('Follow the prompts to install TAH Cafe.'),
            ];
        } else if (isIOS) {
            return [
                localize('Open TAH Cafe in your browser.'),
        ...
        <div className='pwa-install-modal__content'>
            <div className='pwa-install-modal__description'>
                <Text size='xs' color='prominent'>
                    {isPWALaunch
                        ? localize("Welcome to TAH Cafe! You're using the mobile app version.")
                        : isDesktop
                          ? localize('Install TAH Cafe as a desktop app for faster access and a native experience!')
                          : localize(
                                "We're excited to announce that TAH Cafe platform is now a Progressive Web App (PWA)!"
                            )}
                </Text>
            </div>
    ...
    const modalContent = isMobile ? (
        <MobileFullPageModal
            ...
            page_header_text={localize('Install TAH Cafe')}
        ...
        <Modal
            ...
            title={isDesktop ? localize('Install TAH Cafe') : localize('Introducing our installable app')}
            has_close_icon={true}
