...
export default class AppStore {
    ...
    getErrorForNonEuClients = () => ({
        ...
        title: localize('TAH Cafe is unavailable for this account'),
    ...
    getErrorForEuClients = (is_logged_in = false, country: string | undefined = undefined) => {
        return {
            text: ' ',
            title: is_logged_in
                ? localize(`TAH Cafe is not available for ${country || 'EU'} clients`)
                : localize(`TAH Cafe is unavailable in ${country || 'the EU'}`),
